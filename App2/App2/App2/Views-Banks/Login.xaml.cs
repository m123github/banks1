...
namespace App2
{
    ...
    public partial class Login : ContentPage
    {
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (Encryption.DecryptX(jo["STATUS"].ToString()) == "OK" || Encryption.DecryptX(jo["STATUS"].ToString()) == "SUCCESS")
                    {
                        ...
                        cust = new Customer { mlabel = mkey, mvalue = detailsList1[0], NAME = detailsList1[1], HOUSE = detailsList1[2], STREET = detailsList1[3], CITY = detailsList1[3], MOBILE = detailsList1[4], EMAIL = detailsList1[5] };
        ...
        private async void btnFORGOTPIN_Clicked(object sender, EventArgs e)
        {
            try
            {
                ...
                await Task.Delay(1000);
                HttpClient client = new HttpClient();
                string l_strUrl = "https://naiveplusmobileappadv.in/api/passbook/SendPin";
                DataTable l_dtSetupDetailed = new DataTable();
                string l_strCustomerid = BranchID.Text.Replace("/", "").Trim() + "00000" + CustId.Text;
                // string l_strPassword = "1234";
                // string l_strPassword = Pin.Text;
        ...
        private async void btnFORGOTPIN_Clicked_1(object sender, EventArgs e)
        {
            try
            {
                UserDialogs.Instance.ShowLoading("Signing in", MaskType.Gradient);
